const ITEMS = [
  {
    ...
    key: 'outletScoreAvg',
    desc: (s: number) =>
      s >= 4 ? '모든 좌석마다 콘센트가 있어요.' :
      s >= 3 ? '콘센트가 있지만 자리를 잘 골라야 해요.' :
      s >= 2 ? '콘센트가 많지 않아요.' :
               '콘센트 이용이 어려울 수 있어요.',
  },
  {
    ...
    key: 'wifiScoreAvg',
    desc: (s: number) =>
      s >= 4 ? '빠르고 안정적인 와이파이를 제공해요.' :
      s >= 3 ? '와이파이 연결이 가능해요.' :
      s >= 2 ? '와이파이가 불안정할 수 있어요.' :
               '와이파이 이용이 어려울 수 있어요.',
  },
  {
    label: '넓은 좌석',
    ...
    key: 'seatScoreAvg',
    desc: (s: number) =>
      s >= 4 ? '카페가 넓고 쾌적한 공간이 충분해요.' :
      s >= 3 ? '좌석이 적당히 여유 있어요.' :
      s >= 2 ? '좌석이 다소 좁거나 적을 수 있어요.' :
  ...
  },
  {
    ...
    key: 'toiletScoreAvg',
    desc: (s: number) =>
      s >= 4 ? '깨끗하고 쾌적한 화장실이 내부에 있어요.' :
      s >= 3 ? '화장실이 잘 관리되고 있어요.' :
      s >= 2 ? '화장실 상태가 평범한 편이에요.' :
               '화장실 이용 시 불편할 수 있어요.',
  },
] as const;
